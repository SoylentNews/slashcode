...
sub createComment {
	...
	$comment->{signature} = md5_hex( $constants->{utf8} ? encode_utf8($comment_text) : $comment_text );
	$comment->{-date} = 'NOW()';
...
sub createSubmission {
	...
	#$data->{signature} = md5_hex( $constants->{utf8} ? encode_utf8($data->{story}) : $data->{story} );
	my $md5_sig = $data->{story} . $data->{subj};
	$data->{signature} = md5_hex( $constants->{utf8} ? encode_utf8($md5_sig) : $md5_sig );
...
sub findCommentsDuplicate {
	...
	my $signature_quoted = $self->sqlQuote(md5_hex(
		getCurrentStatic('utf8') ? encode_utf8($comment) : $comment
	));
